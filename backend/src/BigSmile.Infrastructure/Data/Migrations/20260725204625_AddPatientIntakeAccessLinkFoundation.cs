        ...
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.CreateTable(
                ...
                columns: table => new
                {
                    ...
                    PatientIntakeId = table.Column<Guid>(type: "uniqueidentifier", nullable: true),
                    RowVersion = table.Column<byte[]>(type: "rowversion", rowVersion: true, nullable: false)
                },
                constraints: table =>
                {
                    table.PrimaryKey("PK_PatientIntakeAccessLinks", x => x.Id);
                    table.CheckConstraint("CK_PatientIntakeAccessLinks_ConsumedAtWindow", "[ConsumedAtUtc] IS NULL OR ([ConsumedAtUtc] >= [CreatedAtUtc] AND [ConsumedAtUtc] < [ExpiresAtUtc])");
                    table.CheckConstraint("CK_PatientIntakeAccessLinks_ConsumptionState", "(([ConsumedAtUtc] IS NULL AND [ConsumedByPatientPortalAccountId] IS NULL AND [PatientIntakeId] IS NULL) OR ([ConsumedAtUtc] IS NOT NULL AND [ConsumedByPatientPortalAccountId] IS NOT NULL AND [PatientIntakeId] IS NOT NULL))");
                    table.CheckConstraint("CK_PatientIntakeAccessLinks_ExpiryOrder", "[ExpiresAtUtc] > [CreatedAtUtc]");
                    table.CheckConstraint("CK_PatientIntakeAccessLinks_RevocationState", "(([RevokedAtUtc] IS NULL AND [RevokedByUserId] IS NULL) OR ([RevokedAtUtc] IS NOT NULL AND [RevokedByUserId] IS NOT NULL))");
                    table.CheckConstraint("CK_PatientIntakeAccessLinks_RevokedAtWindow", "[RevokedAtUtc] IS NULL OR ([RevokedAtUtc] >= [CreatedAtUtc] AND [RevokedAtUtc] < [ExpiresAtUtc])");
                    table.CheckConstraint("CK_PatientIntakeAccessLinks_TerminalState", "NOT ([RevokedAtUtc] IS NOT NULL AND [ConsumedAtUtc] IS NOT NULL)");
                    table.ForeignKey(
                    ...
                    table.ForeignKey(
                        name: "FK_PatientIntakeAccessLinks_PatientIntakes_PatientIntakeId",
            ...
            migrationBuilder.CreateIndex(
                name: "IX_PatientIntakeAccessLinkAuditEntries_TenantId_BranchId_OccurredAtUtc",
                table: "PatientIntakeAccessLinkAuditEntries",
            ...
            migrationBuilder.CreateIndex(
                ...
                unique: true,
                filter: "[ConsumedByPatientPortalAccountId] IS NOT NULL");

            migrationBuilder.CreateIndex(
                name: "IX_PatientIntakeAccessLinks_PatientIntakeId",
                table: "PatientIntakeAccessLinks",
                column: "PatientIntakeId",
                unique: true,
                filter: "[PatientIntakeId] IS NOT NULL");
