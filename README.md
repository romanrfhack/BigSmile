...
* **Accepted Phase 2 slices:** **Phase 2.1 — Appointment Confirmation Foundation**, **Phase 2.2 — Manual Reminder Log Foundation**, **Phase 2.3 — Reminder Scheduling Preparation**, **Phase 2.4 — Reminder Worklist Follow-up Actions**, **Phase 2.5 — Reminder Template Draft Foundation**, and **Phase 2.6 — Reminder Template Usage Traceability**
...
Phase 2.6 — Reminder Template Usage Traceability is accepted. It only adds internal traceability for manual follow-ups that originated from a Phase 2.5 internal template: `AppointmentReminderLogEntry` can store nullable `reminderTemplateId` and `reminderTemplateNameSnapshot`. When there is no template source, both fields remain null; when a template is used, `reminderTemplateNameSnapshot` is derived server-side from the active template and is not accepted from the client. `notes` remains the final manual text; BigSmile does not store a separate template body snapshot, rendered body snapshot, `TemplateVersion`, `DeliveryTemplateId`, `ProviderTemplateId`, or delivery metadata. This does not close Phase 2, adds no new permissions, and still does not send WhatsApp, email, or SMS or add providers, jobs, queues, webhooks, external delivery templates, real scheduler workflows, retry automation, campaigns, patient portal, or online booking.
...
* Accepted slices: Phase 2.1 — Appointment Confirmation Foundation, Phase 2.2 — Manual Reminder Log Foundation, Phase 2.3 — Reminder Scheduling Preparation, Phase 2.4 — Reminder Worklist Follow-up Actions, Phase 2.5 — Reminder Template Draft Foundation, and Phase 2.6 — Reminder Template Usage Traceability
...
* Phase 2.6 is accepted; it adds only nullable internal template trace fields on manual reminder log entries when follow-up uses a template preview as note source, keeps both fields null without a template source, derives the template name snapshot server-side, and does not accept the snapshot from the client
* Phase 2.6 keeps `notes` as the final manual text and stores no body snapshot, rendered snapshot, `TemplateVersion`, `DeliveryTemplateId`, `ProviderTemplateId`, or delivery metadata
