...
- implemented in repo, pending acceptance: `Phase 2.2 — Manual Reminder Log Foundation`
...
Within `Phase 2 Expansion — Modern Operations`, treat `Phase 2.1 — Appointment Confirmation Foundation` as accepted and `Phase 2.2 — Manual Reminder Log Foundation` as implemented in repo but pending acceptance; do not assume WhatsApp, email, SMS, reminders automáticos, online booking, patient portal, templates, campaigns, jobs, queues, webhooks, dashboard avanzado, or later Phase 2 slices are implemented or accepted unless the actual codebase and aligned documentation explicitly prove it.
...
- preserve Phase 2.2 as implemented but pending acceptance: manual contact-attempt log per existing appointment, minimal `Phone` / `WhatsApp` / `Email` / `Other` channel catalog used only as manual record, minimal `Reached` / `NoAnswer` / `LeftMessage` outcome catalog, short optional notes, `CreatedAtUtc` / `CreatedByUserId`, newest-first read, reuse of `scheduling.read` / `scheduling.write`, no new permissions, and no automatic changes to `AppointmentStatus` or `AppointmentConfirmationStatus`
