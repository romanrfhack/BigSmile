...
Treat `Phase 2.6 — Reminder Template Usage Traceability` as implemented in repo and pending acceptance: it only adds nullable internal template trace fields (`ReminderTemplateId`, `ReminderTemplateNameSnapshot`) to manual reminder log entries when follow-up uses an internal template source; `notes` remains the final manual text. Do not treat this as messaging delivery, body/rendered snapshot storage, template versioning, provider integration, notification automation, external delivery templates, or Phase 2 closure.
...
- preserve the implemented Phase 2.6 scope pending acceptance: internal traceability only for manual follow-ups that used an internal reminder template source, nullable `ReminderTemplateId` and `ReminderTemplateNameSnapshot` on `AppointmentReminderLogEntry`, no client-supplied template name snapshot, no body/rendered snapshot, no template versioning, no delivery metadata, reuse of `scheduling.read` / `scheduling.write`, and no new permissions
