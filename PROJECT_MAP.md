...
* preserving the accepted Release 3.5 scope: backend-only fixed medical questionnaire on an existing clinical record, `ClinicalMedicalAnswer` tenant-owned/patient-owned records, allowed `QuestionKey` catalog, `Unknown` / `Yes` / `No` answers, optional bounded details, `GET` / `PUT /api/patients/{patientId}/clinical-record/questionnaire`, no request-provided `TenantId`, no permissions beyond `clinical.read` / `clinical.write`, and no form builder or cross-module side effects
