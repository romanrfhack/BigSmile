...
* **Phase 2 Expansion — Modern Operations:** open in progress through accepted slices **Phase 2.1 — Appointment Confirmation Foundation**, **Phase 2.2 — Manual Reminder Log Foundation**, **Phase 2.3 — Reminder Scheduling Preparation**, **Phase 2.4 — Reminder Worklist Follow-up Actions**, and **Phase 2.5 — Reminder Template Draft Foundation**; **Phase 2.6 — Reminder Template Usage Traceability** is implemented in repo and pending acceptance
...
* treating **Phase 2.6 — Reminder Template Usage Traceability** as implemented in repo and pending acceptance: nullable `ReminderTemplateId` and `ReminderTemplateNameSnapshot` on manual reminder log entries when follow-up uses an internal template source, `notes` remains the final manual text, no body/rendered snapshots, no template versioning, no delivery metadata, existing `scheduling.read` / `scheduling.write`, and no new permissions
...
* keeping WhatsApp/email/SMS sending, external providers, background jobs, queues, webhooks, external delivery templates, real scheduler workflows, retry automation, campaigns, online booking, patient portal, delivery status, and advanced notification automation outside the implemented Phase 2.6 scope
